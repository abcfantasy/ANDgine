...
void Camera::rotate( float x, float y, float deltaT ) {
	...
	this->view_[1] += x * deltaT;
	
	// Limit the rotation so it doesn't go too high or too low
	if(this->view_[1] > this->position_[1] + 2.0f)	this->view_[1] = this->position_[1] + 2.0f;
	if(this->view_[1] < this->position_[1] - 2.0f)	this->view_[1] = this->position_[1] - 2.0f;
	
	// Rotation around the Y-axis
	y = float(M_PI) * ( -y * deltaT ) / 180;
...
void Camera::translate( float x, float y, float z, float deltaT ) {
	...
	this->view_[0] += x * deltaT;
	this->view_[1] += y * deltaT;
	this->view_[2] += z * deltaT;
};
...
// We're not rendering anything visible; we're just moving the camera
void Camera::render() {
