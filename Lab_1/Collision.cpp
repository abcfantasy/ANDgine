...
float* Collision::GetFarthestPointInDirection( std::vector<float*> *shape, float* direction )
{
	float* farthestPoint = (*shape)[0];
	float highest = Math::dotProduct( (*shape)[0], direction );
	float current = 0;

	for ( std::vector<float*>::iterator i = ( shape->begin() + 1 ); i != shape->end(); ++i ) {
		current = Math::dotProduct( (*i), direction );
		if ( current >= highest ) {
			highest = current;
			farthestPoint = (*i);
		}
...
void Collision::support( std::vector<float*> *shape1Vertices, std::vector<float*> *shape2Vertices, float* d, float* result )
{
...
bool Collision::containsOrigin( std::vector<float*> *s, float* d )
{
...
bool Collision::GJKCollide( std::vector<Vertex3f>* shape1, std::vector<Vertex3f>* shape2 )
{
